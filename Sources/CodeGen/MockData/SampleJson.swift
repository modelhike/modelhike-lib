...
public struct SampleJson {
    ...
    public func string(openCloseBraces: Bool = false, openCloseQuotesInNames: Bool = true) async
        -> String
    {
        return await Self.toObjectString(
            entity, typesModel: self.typesModel, openCloseBraces: openCloseBraces,
            openCloseQuotesInNames: openCloseQuotesInNames)
    }

    static func toObjectString(
        _ object: CodeObject, typesModel: ParsedTypesCache, openCloseBraces: Bool,
        openCloseQuotesInNames: Bool
    ) async -> String {
        ...
        return await StringTemplate {
            ...
            for (index, prop) in reqdProperties.enumerated() {
                ...
                await Self.toPropString(
                    prop, typesModel: typesModel, includeComma: includeComma,
                    openCloseQuotesInNames: openCloseQuotesInNames)
    ...
    static func toPropString(
        _ prop: Property, typesModel: ParsedTypesCache, includeComma: Bool,
        openCloseQuotesInNames: Bool
    ) async -> String {
        ...
        case .customType(let typeName):
            ...
            if await prop.type.isArray {
                return await prefix
                    + Self.toObjectString(
                        entity, typesModel: typesModel, openCloseBraces: true,
                        openCloseQuotesInNames: openCloseQuotesInNames) + suffix
            } else {
                return await prefix
                    + Self.toObjectString(
                        entity, typesModel: typesModel, openCloseBraces: true,
