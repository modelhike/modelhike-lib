...
public enum AttachedSectionParser {
    ...
    public static func parse(for obj: ArtifactHolderWithAttachedSections, with pctx: ParsedInfo)
        async throws -> AttachedSection?
    {
        let line = pctx.line.dropFirstWord()
        guard let match = line.wholeMatch(of: ModelRegEx.attachedSectionName_Capturing) else {
            return nil
        }
        ...
        while await parser.linesRemaining {
            ...
            guard let pctx = await parser.currentParsedInfo(level: pctx.level) else {
                await parser.skipLine()
                continue
            }
