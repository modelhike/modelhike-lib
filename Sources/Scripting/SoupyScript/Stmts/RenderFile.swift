...
public struct RenderTemplateFileStmt: LineTemplateStmt, CallStackable, CustomDebugStringConvertible
{
    ...
    public func execute(with ctx: Context) async throws -> String? {
        ...
        if try await !context.events.canRender(
            filename: filename, templatename: self.FromTemplate, with: pInfo)
        {
            return nil
        }
        ...
        await recordDebugFileGenerated(
            in: context, outputPath: filename, templateName: fromTemplate)

        if (try await context.fileGenerator.generateFile(
            filename, template: fromTemplate, with: pInfo)) != nil
        {
