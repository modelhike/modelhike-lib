    ...
    public func execute(with ctx: Context) async throws -> String? {
        ...
        guard let fromFolder = try? await ctx.evaluate(value: FromFolder, with: pInfo) as? String
            ...
            throw Suggestions.variableOrPropertyNotFound(
                FromFolder, candidates: candidates, pInfo: pInfo)
        }
        ...
        foldername =
            try await ContentHandler.eval(expression: foldername, pInfo: pInfo) ?? foldername

        await ctx.debugLog.renderingFolder(fromFolder, to: foldername)
        let _ = try await context.fileGenerator.renderFolder(
            fromFolder, to: foldername, with: pInfo)
