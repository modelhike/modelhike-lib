...
public struct SetStrVarStmt: BlockOrLineTemplateStmt {
        ...
        if await isBlockVariant {
            ...
            if let body = try await children.execute(with: ctx) {
                let modifiedBody = try await Modifiers.apply(
                    to: body.trim(), modifiers: self.ModifiersList, with: pInfo)
    ...
    public init(parseTill endKeyWord: String, pInfo: ParsedInfo) {
        state = BlockOrLineTemplateStmtState(
            keyword: Self.START_KEYWORD, endKeyword: endKeyWord, pInfo: pInfo)
    }

    static let register = BlockOrLineTemplateStmtConfig(
        keyword: START_KEYWORD, endKeyword: "end-set"
    ) { endKeyWord, pInfo in
